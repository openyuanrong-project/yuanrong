...
func TestInstancesHandler(t *testing.T) {
	convey.Convey("Test InstancesHandler:", t, func() {
		r := gin.Default()
		r.GET("/instances", InstancesHandler)
		req, err := http.NewRequest("GET", "/instances", nil)
		convey.So(err, convey.ShouldBeNil)
		w := httptest.NewRecorder()

		convey.Convey("Test InstancesHandler when function master error", func() {
			r.ServeHTTP(w, req)
		...
		convey.Convey("Test Instances success", func() {
			r.ServeHTTP(w, req)
...
func TestGetInstanceStatus(t *testing.T) {
	convey.Convey("Test getInstanceStatus:", t, func() {
		convey.Convey("Test getInstanceStatus when code not in range", func() {
			convey.So(getInstanceStatus(-2), convey.ShouldEqual, "")
		})
	})
}

...
import org.yuanrong.function.GoInstanceClass;
...
public class TestGoInstanceCreator {
    ...
    public void testGoInstanceHandlerInvoke() {
        GoInstanceClass goInstanceClass = GoInstanceClass.of("Counter");
        Assert.assertEquals("Counter", goInstanceClass.className);
        InvokeOptions invokeOptions = new InvokeOptions();
        invokeOptions.setCpu(1500);
        invokeOptions.setMemory(1500);
        GoInstanceCreator goInstanceCreator = YR.instance(goInstanceClass).options(invokeOptions);
        Assert.assertNotNull(goInstanceCreator);
    }

    @Test
    public void testInitGoInstanceCreator() throws IOException, YRException {
        GoInstanceClass goInstanceClass = GoInstanceClass.of("Counter");
        GoInstanceCreator goInstanceCreator1 = new GoInstanceCreator(goInstanceClass);
        goInstanceCreator1.invoke(goInstanceClass);
        goInstanceCreator1.getFunctionMeta();
        Assert.assertEquals("Counter", goInstanceClass.className);
    }

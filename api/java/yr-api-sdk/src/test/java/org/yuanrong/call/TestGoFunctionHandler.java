...
import org.yuanrong.function.GoFunction;
...
public class TestGoFunctionHandler {
    ...
    @Test
    public void testGoInstanceHandlerInvoke() throws IOException, YRException {
        GoFunction<Integer> goFunction = GoFunction.of("PlusOne", int.class, 1);
        GoFunction<Object> plusOne = GoFunction.of("PlusOne", 1);
        Assert.assertEquals("PlusOne", goFunction.functionName);
        Assert.assertEquals(int.class, goFunction.returnType);
        Assert.assertEquals(1, goFunction.returnNum);
        GoFunctionHandler<Integer> goFunctionHandler = YR.function(goFunction);
        InvokeOptions invokeOptions = new InvokeOptions();
        invokeOptions.setCpu(1500);
        invokeOptions.setMemory(1500);
        goFunctionHandler.options(invokeOptions);
        goFunctionHandler.invoke(goFunction);
    }
